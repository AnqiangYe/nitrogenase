...
if len(XX) == 0 or len(YY) == 0:
    ...
    for XX0_index, val_index in Kf1.split(XX, YY):
        ...
        train_x = XX0
        train_y = YY0
        base_model_svr = GradientBoostingRegressor(n_estimators=50, learning_rate=0.01, max_depth=5,random_state=42)
        clf = base_model_svr
    ...
    temp = [np.mean(train_r2), np.mean(val_r2), np.mean(train_spear), np.mean(val_spear), np.mean(mae_train),
            np.mean(mae_val), np.mean(mse_train), np.mean(mse_val), np.mean(rmse_train), np.mean(rmse_val)]
    output = open(f'../result.csv', 'a')
    output.write(
