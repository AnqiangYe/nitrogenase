...
output.write('train_ACC,val_ACC,train_bACC,val_bACC,train_AUC,val_AUC,train_Pre,val_Pre,train_Rec,val_Rec,train_f1,val_f1\n')
...
if len(XX) == 0 or len(YY) == 0:
    ...
    for XX0_index, val_index in Kf1.split(XX, YY):
        ...
        clf = LGBMClassifier(num_leaves=31, learning_rate=0.01, n_estimators=100)
        ...
        temp = [np.mean(Sco_1), np.mean(Sco_22), np.mean(ACC_1), np.mean(ACC_22),
                np.mean(AUC_1), np.mean(AUC_22), np.mean(Pre_1), np.mean(Pre_22),
                np.mean(recall_1), np.mean(recall_22), np.mean(f1_1), np.mean(f1_22)]
        print(np.mean(AUC_22), np.mean(Pre_22))

    # Save results to CSV file
    with open(f'../result.csv', 'a') as output:
        output.write(f'{np.mean(Sco_1)},{np.mean(Sco_22)},{np.mean(ACC_1)},{np.mean(ACC_22)},'
