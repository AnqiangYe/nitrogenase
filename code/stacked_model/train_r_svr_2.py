...
XX = df.iloc[:,1025:1939]
YY = df["label_regression"]
df2=pd.read_csv('../pre1.csv')
ZZ=df2["Predicted"]
XX["Predicted"] = ZZ
...
output.write('k,c,g,train_r2,val_r2,train_spear,val_spear,mae_train,mae_val,mse_train,mse_val,rmse_train,rmse_val\n')
k=0
result = []
    ...
    for g in gamma:
        ...
        if len(XX) == 0 or len(YY) == 0:
            ...
            for XX0_index, val_index in Kf1.split(XX, YY):
                ...
                base_model_svr = SVR(kernel='rbf', C=c, gamma=g)
            ...
            temp = [k, c, g, np.mean(train_r2), np.mean(val_r2), np.mean(train_spear),np.mean(val_spear),np.mean(mae_train),np.mean(mae_val),np.mean(mse_train),np.mean(mse_val),np.mean(rmse_train),np.mean(rmse_val)]
            output = open(f'../result.csv', 'a')
            output.write(f'{k},{c},{g},{np.mean(train_r2)},{np.mean(val_r2)},{np.mean(train_spear)},{np.mean(val_spear)},{np.mean(mae_train)},{np.mean(mae_val)},{np.mean(mse_train)},{np.mean(mse_val)},{np.mean(rmse_train)},{np.mean(rmse_val)}\n')
            result.append(temp)
            output.close()
            output.close()
...
r2_test = r2_score(YY, y_pred)
val_mae = mean_absolute_error(YY, y_pred)
val_mse = mean_squared_error(YY, y_pred)
val_rmse = np.sqrt(val_mse)
print("c：",bc)
print("g：",bg)
print("R2：",r2_test)
